...
def Html_postdata(name, state, data1):
    # 建立连接
    cnx = mysql.connector.connect(**config)     # 根据给定的config数据库信息与mysql数据库建立连接
    cur = cnx.cursor(buffered=True)             # 创建一个游标对象用于执行SQL语句时使用

    # 更新数据库数据
    sql_stmt = "UPDATE test.rtudata SET state=%s,data1=%s WHERE name='%s';" % (state, data1, name)  # 设定SQL语句：更改数据库中gsmdata的相应数据
    cur.execute(sql_stmt)                       # 执行SQL更新语句
    cnx.commit()                # 提交到数据库执行
    cur.close()     # 关闭游标对象
    cnx.close()     # 关闭与Mysql数据库的连接
    return "Data:{} Recieved: OK!".format("[$Name:" + name + "$State:" + state + "$Data1:" + data1+"]")
...
def GSM_postdata(name, temp, data):
    ...
    # 查找数据
    sql_stmt = "SELECT * FROM test.rtudata WEHERE name='"+name+"';"    # 设定SQL查询语句：获取数据库中gsmdata的所有数据
    cur.execute(sql_stmt)                    # 执行SQL语句，将获得的查询结果保存在游标对象中
    ...
    # 更新数据库数据
    sql_stmt = "UPDATE test.rtudata SET temp=%s,data1=%s WHERE name='%s';" % (temp, data, name)      # 设定SQL语句：更改数据库中gsmdata的相应数据
    cur.execute(sql_stmt)               # 执行SQL更新语句
    cnx.commit()                # 提交到数据库执行
    cur.close()         # 关闭游标对象
    cnx.close()         # 关闭与Mysql数据库的连接
